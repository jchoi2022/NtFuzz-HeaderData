...
WINRT_EXPORT namespace winrt
{
    ...
    struct slim_condition_variable
    {
        ...
        void wait(slim_mutex& x, T predicate)
        {
            while (!predicate())
            {
                WINRT_VERIFY(WINRT_SleepConditionVariableSRW(&m_cv, x.get(), 0xFFFFFFFF , 0));
            }
        }
        template <typename T>
...
namespace winrt::impl
{
    ...
    inline constexpr hresult error_false{ 1 };
...
WINRT_EXPORT namespace winrt
{
    ...
    hstring to_hstring(T const& value)
    {
        ...
        int const size = WINRT_MultiByteToWideChar(65001 , 0, view.data(), static_cast<int32_t>(view.size()), nullptr, 0);
        if (size == 0)
        {
            return{};
        }
        impl::hstring_builder result(size);
        WINRT_VERIFY_(size, WINRT_MultiByteToWideChar(65001 , 0, view.data(), static_cast<int32_t>(view.size()), result.data(), size));
        return result.to_hstring();
    }
    inline std::string to_string(std::wstring_view value)
    {
        int const size = WINRT_WideCharToMultiByte(65001 , 0, value.data(), static_cast<int32_t>(value.size()), nullptr, 0, nullptr, nullptr);
        if (size == 0)
        {
            return{};
        }
        std::string result(size, '?');
        WINRT_VERIFY_(size, WINRT_WideCharToMultiByte(65001 , 0, value.data(), static_cast<int32_t>(value.size()), result.data(), size, nullptr, nullptr));
        return result;
    }
}
...
WINRT_EXPORT namespace winrt
{
    struct hresult_error
    {
        ...
        hstring message() const noexcept
        {
            ...
            uint32_t const size = WINRT_FormatMessageW(0x00001300,
                nullptr,
                m_code,
                0x00000400,
...
WINRT_EXPORT namespace winrt
{
    ...
    impl::com_ref<Interface> create_instance(guid const& clsid, uint32_t context = 0x1 , void* outer = nullptr)
    {
...
WINRT_EXPORT namespace winrt
{
    struct access_token : handle
    {
        ...
        {
            access_token token;
            check_bool(WINRT_OpenProcessToken(WINRT_GetCurrentProcess(), 0x0002 , token.put()));
            access_token duplicate;
            check_bool(WINRT_DuplicateToken(token.get(), 2 , duplicate.put()));
            return duplicate;
        }
        static access_token thread()
        {
            access_token token;
            if (!WINRT_OpenThreadToken(WINRT_GetCurrentThread(), 0x0004 , 1, token.put()))
            {
...
namespace winrt::impl
{
    inline bool is_sta() noexcept
    {
        ...
        return (error_ok == WINRT_CoGetApartmentType(&aptType, &aptTypeQualifier)) && ((aptType == 0 ) || (aptType == 3 ));
    }
    template <typename Async>
    void blocking_suspend(Async const& async)
    {
